...
do_install () {
    ...
    install -m 0755 "${RECIPE_SYSROOT}${sgxrootdir}/package/sgx_linux_x64_sdk_2.18.100.3.bin" "${D}${sgxdirprefix}${sgxrootdir}/package"
    install -m 0755 "${B}/linux/installer/bin/sgx_linux_x64_psw_2.18.100.3.bin" "${D}${sgxdirprefix}${sgxrootdir}/package"
...
pkg_postinst_ontarget:${PN} () {
    ...
    if [ -z "$D" ]; then
        ...
        if [ -e "${sgxrootdir}/package/sgx_linux_x64_sdk_2.18.100.3.bin" ]; then
            "${sgxrootdir}/package/sgx_linux_x64_sdk_2.18.100.3.bin" --prefix /opt/intel < /dev/null
        fi

        # Install PSW
        if [ -e "${sgxrootdir}/package/sgx_linux_x64_psw_2.18.100.3.bin" ]; then
            "${sgxrootdir}/package/sgx_linux_x64_psw_2.18.100.3.bin" < /dev/null &
            sleep 10
        fi
